
public class TicketOrder {
	static class Builder {
		private String title;
		private int quantityCategory1;
		private int quantityCategory2;
		private int quantityCategory3;
		private int priceCategory1;
		private int priceCategory2;
		private int priceCategory3;
		private String date;
		private int value;
		
		public Builder(){}
		public Builder buy(int qtyCat1, int qtyCat2, int qtyCat3, String title) {
			this.quantityCategory1 = qtyCat1;
			this.quantityCategory2 = qtyCat2;
			this.quantityCategory3 = qtyCat3;
			this.title = title;
			return this;
		}
		
		public Builder atDate(String date) {
			this.date = date;
			return this;
		}
		public Builder atPrice(int pCat1, int pCat2, int pCat3) {
			this.priceCategory1 = pCat1;
			this.priceCategory2 = pCat2;
			this.priceCategory3 = pCat3;
			return this;
		}

		public Builder valueAs(OrderValuer ov) {
			this.value = ov.valueAs(quantityCategory1, quantityCategory2, quantityCategory3, priceCategory1,
					priceCategory2, priceCategory3);
			return this;
		}
		public TicketOrder build() {
			return new TicketOrder(this);
		}
	}
	
	final String title;
	private final int qtyCat1;
	private final int qtyCat2;
	private final int qtyCat3;
	final String date;
	final int value;
	
	private TicketOrder(Builder b) {
		title = b.title;
		qtyCat1 = b.quantityCategory1;
		qtyCat2 = b.quantityCategory2;
		qtyCat3 = b.quantityCategory3;
		date = b.date;
		value = b.value;
	}
}

